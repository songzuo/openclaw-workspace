...
for i in {1..10}; do
  ...
  if echo "$RESPONSE" | python3 -c "import sys,json; d=json.load(sys.stdin); print('ok' if 'choices' in d else 'fail')" 2>/dev/null | grep -q "ok"; then
    ...
    if [ -n "$CONTENT" ]; then
      SUCCESS=$((SUCCESS + 1))
      echo "✅ #$i | ${DURATION}ms | content: ${CONTENT:0:50}"
    elif [ -n "$REASONING" ]; then
      SUCCESS=$((SUCCESS + 1))
      echo "✅ #$i | ${DURATION}ms | reasoning: ${REASONING:0:50}..."
    else
      FAIL=$((FAIL + 1))
      echo "⚠️ #$i | ${DURATION}ms | 空响应"
    fi
  else
    FAIL=$((FAIL + 1))
...
echo ""
echo "===== 📊 统计 (glm-4.7) ====="
echo "✅ 有效: $SUCCESS/10 | ❌ 失败: $FAIL/10 | ⏱️ 平均: $((TOTAL_TIME / 10))ms | 🟢 可用率: $((SUCCESS * 100 / 10))%"
